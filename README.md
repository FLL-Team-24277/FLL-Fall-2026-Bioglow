...
6. Create a new python file named `teamMemberName-test-mission.py`. Copy and paste the code below and save it, but don't try to run it just yet. Wait for the final step below. Note that after saving the file, the python Black Formatter should correct the "incorrect" spacing around the equals signs and comments.
...
9. Create a user variable for the robot name. Set the variable `fllRobotName` to the name of the robot in the user's settings.json. The easy way to do this is to click on the fllRobotName button on the status bar. This should allow the keyboard binding and tasks to recognize the robot by name.
